...
// #include "dev/dht22.h"#include "dev/i2c.h"
...
PROCESS_THREAD(init_system_process, ev, data) {
  ...
  while(1) {
    ...
    //       // sprintf(data_pub, "{\"Press\": \"%u.%u\", \"Temp\": \"%d.%u\"}", pressure / 10, pressure % 10, temperature / 10, temperature % 10);
          ...
          mqtt_sn_pub(topics_mqtt[0] , data_pub, false, 0);
    // // } else {
    // //       // printf("Error, enable the DEBUG flag in the BMPx8x driver for info, ");
    // //       // printf("or check if the sensor is properly connected\n");
          // mqtt_sn_pub(topics_mqtt[0], "Error to reading sensor", false, 0);
    // //       // PROCESS_EXIT();
    // // }
    // mqtt_sn_disconnect(4);
    lpm_enter();
  }   
  PROCESS_END();
}
